...
void controller_base::actuator_controls_publish(const ros::TimerEvent&)
{
    ...
    if(_joy_command_recieved) {
      ...
      if(_joy_commands.mode == ros_pilot::JoyCommand::MODE_DIRECT_CONTROL) {
        output_s output;
        output.delta_a = _joy_commands.x;
        output.delta_e = _joy_commands.y;
        output.delta_r = _joy_commands.z;
        output.delta_t = _joy_commands.F;
      ...
      // Control based off of desired Va, h, and chi (given by joystick)
      else if (_joy_commands.mode == ros_pilot::JoyCommand::MODE_STABLE_CONTROL) {
